...
function ChatPageContent() {
  ...
  return (
    <div className={`h-screen flex flex-col ${isDarkMode ? 'bg-black' : 'bg-gray-100'}`}> {/* Apply dark mode background */}
      <div className="flex flex-1 overflow-hidden">
        ...
        {showConversations && (
          <div className={cn(
            ...
            {/* User Profile Header */}
            <div className={`p-4 border-b border-${isDarkMode ? 'gray-700' : 'gray-200'} flex items-center justify-between bg-primary/5`}>
              <div className="flex items-center">
                ...
                />
                <div>
                  <p className={`font-medium text-${isDarkMode ? 'white' : 'gray-900'}`}>{user?.name}</p>
                  <p className={`text-xs text-${isDarkMode ? 'gray-400' : 'gray-500'}`}>@{user?.username}</p>
                </div>
              </div>
            ...
            {/* Search Bar */}
            <div className={`p-4 border-b border-${isDarkMode ? 'gray-700' : 'gray-200'}`}>
              <div className="relative">
                <Search className={`absolute left-3 top-1/2 transform -translate-y-1/2 text-${isDarkMode ? 'gray-300' : 'gray-400'} h-4 w-4`} />
                <Input
                  type="text"
            ...
            <div className="flex-1 overflow-y-auto">
              {isLoadingConversations ? (
                ...
                Array(5).fill(0).map((_, i) => (
                  <div key={i} className={`p-4 border-b border-${isDarkMode ? 'gray-700' : 'gray-200'} flex items-center`}>
                ...
                <div className={`p-8 text-center text-${isDarkMode ? 'gray-400' : 'gray-500'}`}>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <h3 className={`text-sm font-medium text-${isDarkMode ? 'white' : 'gray-900'} truncate`}>
                          {conversation.otherUser.name}
                        </h3>
                        <span className={`text-xs text-${isDarkMode ? 'gray-400' : 'gray-500'}`}>
                          {conversation.lastMessage && format(new Date(conversation.lastMessage.timestamp), 'h:mm a')}
                        </span>
                      </div>
                      <div className="flex justify-between items-center mt-1">
                        <p className={`text-xs text-${isDarkMode ? 'gray-400' : 'gray-500'} truncate`}>
        ...
        {showChat && (
          <div className={cn(
            ...
            {activeConversation ? (
              <>
                {/* Chat Header */}
                <div className={`p-4 border-b border-${isDarkMode ? 'gray-700' : 'gray-200'} bg-${isDarkMode ? 'black' : 'white'} flex items-center justify-between`}>
                  <div className="flex items-center">
                    {isMobile && (
                      <button 
                    ...
                    />
                    <div>
                      <p className={`font-medium text-${isDarkMode ? 'white' : 'gray-900'}`}>
                        {activeUser?.name}
                      </p>
                      <div className="flex items-center">
                        <span className={cn(
                ...
                >
                  ...
                  <div className="flex justify-center mb-4">
                    <span className={`text-xs text-${isDarkMode ? 'gray-400' : 'gray-500'} bg-${isDarkMode ? 'gray-800' : 'gray-100'} rounded-full px-3 py-1`}>
                      {messages.length > 0 
                  ...
                  {isLoadingMessages ? (
                    ...
                    <div className={`text-center text-${isDarkMode ? 'gray-400' : 'gray-500'} my-12`}>
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            <p className={`text-${isDarkMode ? 'white' : 'gray-900'}`}>{message.content}</p>
                            <div className="flex items-center justify-end mt-1 gap-1">
                              <p 
                                className={cn(
                                  "text-xs",
                                  isSent ? `text-${isDarkMode ? 'white' : 'primary-100'}` : `text-${isDarkMode ? 'gray-400' : 'gray-500'}`
                                )}
                              >
                              ...
                              {isSent && (
                                <span className={cn(
                                  "text-xs",
                                  message.read ? "text-blue-400" : `text-${isDarkMode ? 'white' : 'primary-100'}`
                                )}>
                                  {message.read ? "✓✓" : "✓"}
                                </span>
                              )}
                  ...
                  {activeUser && usersTyping[activeUser.id.toString()] && (
                    <div className="flex mb-4">
                      <AvatarWithStatus
                        ...
                        className="mr-2 self-end"
                      />
                      <div className={`bg-${isDarkMode ? 'gray-800' : 'white'} rounded-lg py-2 px-4 shadow-sm`}>
                        <div className="flex space-x-1">
                          <div className={`w-2 h-2 bg-${isDarkMode ? 'gray-400' : 'gray-400'} rounded-full animate-bounce`}></div>
                          <div className={`w-2 h-2 bg-${isDarkMode ? 'gray-400' : 'gray-400'} rounded-full animate-bounce`} style={{ animationDelay: '0.2s' }}></div>
                          <div className={`w-2 h-2 bg-${isDarkMode ? 'gray-400' : 'gray-400'} rounded-full animate-bounce`} style={{ animationDelay: '0.4s' }}></div>
                        </div>
                      </div>
                    </div>
                  )}
                ...
                {/* Message Input */}
                <div className={`p-4 border-t border-${isDarkMode ? 'gray-700' : 'gray-200'} bg-${isDarkMode ? 'black' : 'white'}`}>
              ...
              <div className="flex flex-col items-center justify-center h-full">
                <div className={`text-center max-w-md p-8 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  <div className="bg-primary/10 p-4 rounded-full inline-block mb-4">
                    <MessageSquare className="h-12 w-12 text-primary" />
                  </div>
                  <h2 className={`text-2xl font-bold mb-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Select a conversation</h2>
                  <p className={`text-${isDarkMode ? 'gray-400' : 'gray-500'} mb-6`}>
        ...
        {!showChat && !isMobile && (
          <div className={`flex-1 bg-${isDarkMode ? 'black' : 'gray-50'} flex items-center justify-center`}>
            <div className={`text-center max-w-md p-8 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              <div className="bg-primary/10 p-4 rounded-full inline-block mb-4">
                <MessageSquare className="h-12 w-12 text-primary" />
              </div>
